...
import svg_data as sd


# local path
#STAC_DATA_DIR = '/home/mmorey/melodi/stac/svn-stac/data'
STAC_DATA_DIR = '/Users/kthompson/stac_fork/irit-stac/data'
...
def read_game_as_dataframes(game_folder, sel_annotator=None, thorough=True):
    ...
    #print("upfolder", game_upfolder, "game name", game_name)
    game_corpus = StacReader(game_upfolder).slurp(doc_glob=game_name)
    #print("game corpus", game_corpus)
    for doc_key, doc_val in sorted(game_corpus.items()):
        ...
        for anno in doc_val.relations:
            ...
            rel_dict.update({
                ...
                'source_type': anno.source.type,
                'target_type': anno.target.type,
            })
...
def get_seg_orders(segments_table):
    subset = segments_table[['doc', 'subdoc']]
    subdocs = [tuple(x) for x in subset.values]

    for subdoc in subdocs:
        subset = segs_situ.loc[(segs_situ['doc'] == subdoc[0]) & (segs_situ['subdoc'] == subdoc[1])][['global_id', 'type', 'span_end']].sort_values('span_end')
        segments = [tuple(x) for x in subset.values]
...
if __name__ == '__main__':
    ...
    """
    for all games:
    -create tables and save as pickle files to be opened by jupyter notebook
    -create intermediary tables and pickle them
    -create objects used to create svg visualizations and pickle them
